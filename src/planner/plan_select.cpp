...
auto Planner::PlanSelect(const SelectStatement &statement) -> AbstractPlanNodeRef {
    ...
    switch (statement.table_->type_) {
        case TableReferenceType::EMPTY:
            ...
            break;
        default: plan = PlanTableRef(*statement.table_); break;
    }
    ...
    if (has_window_agg) {
        ...
        plan =
            std::make_shared<ProjectionPlanNode>(std::make_shared<Schema>(ProjectionPlanNode::RenameSchema(
    ...
    if (!statement.sort_.empty()) {
        ...
        plan =
            std::make_shared<SortPlanNode>(std::make_shared<Schema>(plan->OutputSchema()), plan, std::move(order_bys));
    }
