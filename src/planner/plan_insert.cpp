...
auto Planner::PlanInsert(const InsertStatement &statement) -> AbstractPlanNodeRef {
    ...
    auto insert_schema =
        std::make_shared<Schema>(std::vector{Column("__bustub_internal.insert_rows", TypeId::INTEGER)});
...
auto Planner::PlanDelete(const DeleteStatement &statement) -> AbstractPlanNodeRef {
    ...
    auto delete_schema =
        std::make_shared<Schema>(std::vector{Column("__bustub_internal.delete_rows", TypeId::INTEGER)});
...
auto Planner::PlanUpdate(const UpdateStatement &statement) -> AbstractPlanNodeRef {
    ...
    auto update_schema =
        std::make_shared<Schema>(std::vector{Column("__bustub_internal.update_rows", TypeId::INTEGER)});
