...
auto TableGenerator::GenNumericValues(ColumnInsertMeta *col_meta, uint32_t count) -> std::vector<Value> {
    ...
    if (col_meta->dist_ == Dist::Serial) {
        for (uint32_t i = 0; i < count; i++) {
            values.emplace_back(
                Value(col_meta->type_, static_cast<CppType>(col_meta->serial_counter_ + col_meta->min_)));
...
auto TableGenerator::MakeValues(ColumnInsertMeta *col_meta, uint32_t count) -> std::vector<Value> {
    ...
    switch (col_meta->type_) {
        case TypeId::TINYINT: return GenNumericValues<int8_t>(col_meta, count);
        case TypeId::SMALLINT: return GenNumericValues<int16_t>(col_meta, count);
        case TypeId::INTEGER: return GenNumericValues<int32_t>(col_meta, count);
        case TypeId::BIGINT: return GenNumericValues<int64_t>(col_meta, count);
        case TypeId::DECIMAL: return GenNumericValues<double>(col_meta, count);
        default: UNREACHABLE("Not yet implemented");
    }
}
...
void TableGenerator::GenerateTestTables() {
    ...
    std::vector<TableInsertMeta> insert_meta{
        ...
        // {"test_3",
        //  TEST3_SIZE,
        //  {{"colA", TypeId::INTEGER, false, Dist::Serial, 0, 0}, {"colB", TypeId::INTEGER, true, Dist::Serial, 0,
        //  0}}},
        ...
        // {"empty_table3",
        //  0,
        //  {{"colA", TypeId::BIGINT, false, Dist::Serial, 0, 0}, {"colB", TypeId::INTEGER, false, Dist::Uniform, 0,
        //  9}}},
    };
