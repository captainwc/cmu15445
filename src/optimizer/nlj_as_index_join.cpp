            ...
            if (expr->comp_type_ == ComparisonType::Equal) {
                if (const auto *left_expr = dynamic_cast<const ColumnValueExpression *>(expr->children_[0].get());
                    ...
                    if (const auto *right_expr = dynamic_cast<const ColumnValueExpression *>(expr->children_[1].get());
                        ...
                        auto left_expr_tuple_0  = std::make_shared<ColumnValueExpression>(0, left_expr->GetColIdx(),
                                                                                         left_expr->GetReturnType());
                        auto right_expr_tuple_0 = std::make_shared<ColumnValueExpression>(0, right_expr->GetColIdx(),
                        ...
                        if (nlj_plan.GetRightPlan()->GetType() == PlanType::SeqScan) {
                            const auto &right_seq_scan =
                                dynamic_cast<const SeqScanPlanNode &>(*nlj_plan.GetRightPlan());
                            if (left_expr->GetTupleIdx() == 0 && right_expr->GetTupleIdx() == 1) {
                                if (auto index = MatchIndex(right_seq_scan.table_name_, right_expr->GetColIdx());
                                    ...
                                    return std::make_shared<NestedIndexJoinPlanNode>(
                                        ...
                                        right_seq_scan.GetTableOid(), index_oid, std::move(index_name),
                                        right_seq_scan.table_name_, right_seq_scan.output_schema_,
