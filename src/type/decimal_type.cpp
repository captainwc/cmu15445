...
#define DECIMAL_COMPARE_FUNC(OP)                                                                  \
    switch (right.GetTypeId()) {                                                                  \
        case TypeId::TINYINT: return GetCmpBool(left.value_.decimal_ OP right.GetAs<int8_t>());   \
        case TypeId::SMALLINT: return GetCmpBool(left.value_.decimal_ OP right.GetAs<int16_t>()); \
        case TypeId::INTEGER: return GetCmpBool(left.value_.decimal_ OP right.GetAs<int32_t>());  \
        case TypeId::BIGINT: return GetCmpBool(left.value_.decimal_ OP right.GetAs<int64_t>());   \
        case TypeId::DECIMAL: return GetCmpBool(left.value_.decimal_ OP right.GetAs<double>());   \
        case TypeId::VARCHAR: {                                                                   \
...
#define DECIMAL_MODIFY_FUNC(OP)                                                                               \
    switch (right.GetTypeId()) {                                                                              \
        case TypeId::TINYINT: return Value(TypeId::DECIMAL, left.value_.decimal_ OP right.GetAs<int8_t>());   \
        case TypeId::SMALLINT: return Value(TypeId::DECIMAL, left.value_.decimal_ OP right.GetAs<int16_t>()); \
        case TypeId::INTEGER: return Value(TypeId::DECIMAL, left.value_.decimal_ OP right.GetAs<int32_t>());  \
        case TypeId::BIGINT: return Value(TypeId::DECIMAL, left.value_.decimal_ OP right.GetAs<int64_t>());   \
        case TypeId::DECIMAL: return Value(TypeId::DECIMAL, left.value_.decimal_ OP right.GetAs<double>());   \
        case TypeId::VARCHAR: {                                                                               \
            auto r_value = right.CastAs(TypeId::DECIMAL);                                                     \
...
auto DecimalType::Modulo(const Value &left, const Value &right) const -> Value {
    ...
    switch (right.GetTypeId()) {
        case TypeId::TINYINT: return {TypeId::DECIMAL, ValMod(left.value_.decimal_, right.GetAs<int8_t>())};
        case TypeId::SMALLINT: return {TypeId::DECIMAL, ValMod(left.value_.decimal_, right.GetAs<int16_t>())};
        case TypeId::INTEGER: return {TypeId::DECIMAL, ValMod(left.value_.decimal_, right.GetAs<int32_t>())};
        case TypeId::BIGINT: return {TypeId::DECIMAL, ValMod(left.value_.decimal_, right.GetAs<int64_t>())};
        case TypeId::DECIMAL: return {TypeId::DECIMAL, ValMod(left.value_.decimal_, right.GetAs<double>())};
        case TypeId::VARCHAR: {
            auto r_value = right.CastAs(TypeId::DECIMAL);
...
auto DecimalType::CastAs(const Value &val, const TypeId type_id) const -> Value {
    switch (type_id) {
        ...
        case TypeId::DECIMAL: return val.Copy();
        case TypeId::VARCHAR: {
            if (val.IsNull()) {
