...
#define TINYINT_COMPARE_FUNC(OP)                                                                  \
    switch (right.GetTypeId()) {                                                                  \
        case TypeId::TINYINT: return GetCmpBool(left.value_.tinyint_ OP right.GetAs<int8_t>());   \
        case TypeId::SMALLINT: return GetCmpBool(left.value_.tinyint_ OP right.GetAs<int16_t>()); \
        case TypeId::INTEGER: return GetCmpBool(left.value_.tinyint_ OP right.GetAs<int32_t>());  \
        case TypeId::BIGINT: return GetCmpBool(left.value_.tinyint_ OP right.GetAs<int64_t>());   \
        case TypeId::DECIMAL: return GetCmpBool(left.value_.tinyint_ OP right.GetAs<double>());   \
        case TypeId::VARCHAR: {                                                                   \
...
#define TINYINT_MODIFY_FUNC(METHOD, OP)                                                                     \
    switch (right.GetTypeId()) {                                                                            \
        ...
        case TypeId::DECIMAL: return Value(TypeId::DECIMAL, left.value_.tinyint_ OP right.GetAs<double>()); \
        case TypeId::VARCHAR: {                                                                             \
            auto r_value = right.CastAs(TypeId::TINYINT);                                                   \
            /* NOLINTNEXTLINE  */                                                                           \
...
auto TinyintType::Modulo(const Value &left, const Value &right) const -> Value {
    ...
    switch (right.GetTypeId()) {
        case TypeId::TINYINT: return ModuloValue<int8_t, int8_t>(left, right);
        case TypeId::SMALLINT: return ModuloValue<int8_t, int16_t>(left, right);
        case TypeId::INTEGER: return ModuloValue<int8_t, int32_t>(left, right);
        case TypeId::BIGINT: return ModuloValue<int8_t, int64_t>(left, right);
        case TypeId::DECIMAL: return {TypeId::DECIMAL, ValMod(left.value_.tinyint_, right.GetAs<double>())};
        case TypeId::VARCHAR: {
            auto r_value = right.CastAs(TypeId::TINYINT);
...
auto TinyintType::OperateNull(const Value &left __attribute__((unused)), const Value &right) const -> Value {
    switch (right.GetTypeId()) {
        case TypeId::TINYINT: return {TypeId::TINYINT, static_cast<int8_t>(BUSTUB_INT8_NULL)};
        case TypeId::SMALLINT: return {TypeId::SMALLINT, static_cast<int16_t>(BUSTUB_INT16_NULL)};
        case TypeId::INTEGER: return {TypeId::INTEGER, static_cast<int32_t>(BUSTUB_INT32_NULL)};
        case TypeId::BIGINT: return {TypeId::BIGINT, static_cast<int64_t>(BUSTUB_INT64_NULL)};
        case TypeId::DECIMAL: return {TypeId::DECIMAL, static_cast<double>(BUSTUB_DECIMAL_NULL)};
        default: break;
    }
    throw Exception("type error");
}
