...
#define INT_COMPARE_FUNC(OP)                                                                      \
    switch (right.GetTypeId()) {                                                                  \
        case TypeId::TINYINT: return GetCmpBool(left.value_.integer_ OP right.GetAs<int8_t>());   \
        case TypeId::SMALLINT: return GetCmpBool(left.value_.integer_ OP right.GetAs<int16_t>()); \
        case TypeId::INTEGER: return GetCmpBool(left.value_.integer_ OP right.GetAs<int32_t>());  \
        case TypeId::BIGINT: return GetCmpBool(left.value_.integer_ OP right.GetAs<int64_t>());   \
        case TypeId::DECIMAL: return GetCmpBool(left.value_.integer_ OP right.GetAs<double>());   \
        case TypeId::VARCHAR: {                                                                   \
...
#define INT_MODIFY_FUNC(METHOD, OP)                                                                         \
    switch (right.GetTypeId()) {                                                                            \
        ...
        case TypeId::DECIMAL: return Value(TypeId::DECIMAL, left.value_.integer_ OP right.GetAs<double>()); \
        case TypeId::VARCHAR: {                                                                             \
            auto r_value = right.CastAs(TypeId::INTEGER);                                                   \
            /* NOLINTNEXTLINE */                                                                            \
...
auto IntegerType::Modulo(const Value &left, const Value &right) const -> Value {
    ...
    switch (right.GetTypeId()) {
        case TypeId::TINYINT: return ModuloValue<int32_t, int8_t>(left, right);
        case TypeId::SMALLINT: return ModuloValue<int32_t, int16_t>(left, right);
        case TypeId::INTEGER: return ModuloValue<int32_t, int32_t>(left, right);
        case TypeId::BIGINT: return ModuloValue<int32_t, int64_t>(left, right);
        case TypeId::DECIMAL: return {TypeId::DECIMAL, ValMod(left.value_.integer_, right.GetAs<double>())};
        case TypeId::VARCHAR: {
            auto r_value = right.CastAs(TypeId::INTEGER);
