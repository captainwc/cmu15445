    ...
    switch (plan->GetType()) {
        ...
        case PlanType::NestedLoopJoin: {
            ...
            if (check_options_set.find(CheckOption::ENABLE_NLJ_CHECK) != check_options_set.end()) {
                auto left_check  = std::make_unique<InitCheckExecutor>(exec_ctx, nested_loop_join_plan->GetLeftPlan(),
                                                                      std::move(left));
                auto right_check = std::make_unique<InitCheckExecutor>(exec_ctx, nested_loop_join_plan->GetRightPlan(),
                                                                       std::move(right));
        ...
        case PlanType::TopN: {
            ...
            if (check_options_set.find(CheckOption::ENABLE_TOPN_CHECK) != check_options_set.end()) {
                ...
                auto check =
                    std::make_unique<TopNCheckExecutor>(exec_ctx, topn_plan, std::move(child), topn_executor.get());
        ...
        default: UNREACHABLE("Unsupported plan type.");
    }
}
