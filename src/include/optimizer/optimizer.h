...
private:
     ...
     * Identical projection might be produced when there's `SELECT *`, aggregation, or when we need to rename the
     * columns in the planner. We merge these projections so as to make execution faster.
     */
     ...
     * In planner, we plan cross join + filter with cross product (done with nested loop join) and a filter plan node.
     * We can merge the filter condition into nested loop join to achieve better efficiency.
     */
     ...
     * In the starter code, we will check NLJs with exactly one equal condition. You can further support optimizing
     * joins with multiple eq conditions.
     */
