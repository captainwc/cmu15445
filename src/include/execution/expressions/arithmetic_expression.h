...
private:
    auto PerformComputation(const Value &lhs, const Value &rhs) const -> std::optional<int32_t> {
        ...
        switch (compute_type_) {
            case ArithmeticType::Plus: return lhs.GetAs<int32_t>() + rhs.GetAs<int32_t>();
            case ArithmeticType::Minus: return lhs.GetAs<int32_t>() - rhs.GetAs<int32_t>();
            default: UNREACHABLE("Unsupported arithmetic type.");
        }
    }
};
}  // namespace bustub
...
struct fmt::formatter<bustub::ArithmeticType> : formatter<string_view> {
    ...
    auto format(bustub::ArithmeticType c, FormatContext &ctx) const {
        string_view name;
        switch (c) {
            case bustub::ArithmeticType::Plus: name = "+"; break;
            case bustub::ArithmeticType::Minus: name = "-"; break;
