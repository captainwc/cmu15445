...
public:
     ...
     * @param table_name The name of the new table, note that all tables beginning with `__` are reserved for the
     * system.
    ...
    auto CreateIndex(Transaction *txn, const std::string &index_name, const std::string &table_name,
        ...
        if (index_type == IndexType::HashTableIndex) {
            ...
            index = std::make_unique<STLUnorderedIndex<KeyType, ValueType, KeyComparator>>(std::move(meta), bpm_,
                                                                                           hash_function);
        } else {
