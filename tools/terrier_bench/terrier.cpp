    ...
    void Report() {
        ...
        if (elapsed - last_report_at_ > 1000) {
            fmt::print(stderr,
                       ...
                       (committed_txn_cnt_ - last_committed_txn_cnt_) / static_cast<double>(elapsed - last_report_at_)
                           * 1000,
...
void Bench2TaskJoin(const int thread_id, const int terrier_num, const uint64_t duration_ms,
    ...
    while (!metrics.ShouldFinish()) {
        ...
        if (join_action == 0) {
            ...
            auto        txn = bustub->txn_manager_->Begin(iso_lvl);
            std::string target_network_sql =
                fmt::format("SELECT network FROM terriers WHERE terrier = {}", join_target);
            ...
            std::string join_sql =
                fmt::format("UPDATE terriers SET network = {}, token = token + {} WHERE terrier = {}", target_network,
                            sign_bonus, terrier);
            ...
            auto        txn = bustub->txn_manager_->Begin(iso_lvl);
            std::string join_sql =
                fmt::format("UPDATE terriers SET network = {}, token = token - {} WHERE terrier = {}", join_target,
                            sign_bonus, terrier);
            if (!bustub->ExecuteSqlTxn(join_sql, writer, txn)) {
                bustub->txn_manager_->Abort(txn);
                metrics.TxnAborted();
        ...
        if (join_action == 2) {
            ...
            std::string join1_sql =
                fmt::format("UPDATE terriers SET network = {}, token = token + {} WHERE terrier = {}", network2,
                            sign_bonus, terrier1);
            std::string join2_sql =
                fmt::format("UPDATE terriers SET network = {}, token = token + {} WHERE terrier = {}", network1,
                            sign_bonus, terrier2);
        ...
        if (bench_1) {
            threads.emplace_back([thread_id, bustub_terrier_num, duration_ms, &bustub, &total_metrics]() {
                Bench1TaskTransfer(thread_id, bustub_terrier_num, duration_ms,
                                   bustub::IsolationLevel::SNAPSHOT_ISOLATION, bustub.get(), total_metrics);
            });
        }
        if (bench_2) {
            if (thread_id % 2 == 0) {
