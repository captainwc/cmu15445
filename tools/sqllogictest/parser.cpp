    ...
    while (line_iter != lines.cend()) {
        ...
        if (tokens[0] == "statement") {
            ...
            records.emplace_back(
                std::make_unique<StatementRecord>(loc, error, std::move(sql), std::move(extra_options)));
            if (line_iter == lines.cend()) {
                break;
            }
        }
        ...
        if (tokens[0] == "query") {
            ...
            StringUtil::RTrim(&sql);
            records.emplace_back(std::make_unique<QueryRecord>(loc, sort_mode, std::move(sql),
                                                               std::move(expected_result), std::move(extra_options)));
            if (line_iter == lines.cend()) {
                break;
            }
        }
