            ...
            if (opt == "ensure:index_scan") {
                ...
                for (auto &line : lines) {
                    ...
                    if (bustub::StringUtil::StartsWith(line, "Agg")) {
                        ...
                        for (int i = 0; i < 2; i++) {
                            if (bustub::StringUtil::Count(cols[i], "\",") + 1
                                > static_cast<size_t>(expected_cols_agg)) {
...
auto main(int argc, char **argv) -> int {  // NOLINT
    ...
    for (const auto &record : result) {
        ...
        switch (record->type_) {
                ...
                try {
                    ...
                    if (!ResultCompare(result.str(), query.expected_result_, query.sort_mode_, diff)) {
                        if (diff) {
                            ...
                            fmt::print("wrong result (with sort_mode={}), use `-d` to store your result and expected "
                                       "result in a file\n",
                                       query.sort_mode_);
                        }
                        return 1;
            ...
            default: throw bustub::Exception("unsupported record");
        }
    }
