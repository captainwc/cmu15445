...
TEST(BufferPoolManagerTest, DISABLED_PagePinMediumTest) {
    ...
    // Scenario: Once we unpin page 0 and then make a new page, all the buffer pages should now be pinned. Fetching page
    // 0 again should fail.
...
TEST(BufferPoolManagerTest, DISABLED_EvictableTest) {
    ...
    for (size_t i = 0; i < rounds; i++) {
        ...
        // This signal tells the readers that they can start reading after the main thread has already taken the read
        // latch.
        bool signal = false;
