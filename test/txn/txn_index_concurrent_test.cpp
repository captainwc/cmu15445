    ...
    for (int n = 0; n < trials; n++) {
        ...
        for (int thread = 0; thread < thread_cnt; thread++) {
            update_threads.emplace_back([add_delete_insert, &bustub, thread, generate_sql, generate_select_sql,
                                         generate_delete_sql, generate_txn_insert_sql, &result_mutex,
                                         &operation_result]() {
                NoopWriter        writer;
                std::vector<bool> result;
                result.reserve(number_cnt);
                for (int i = 0; i < number_cnt; i++) {
                    ...
                    if (add_delete_insert) {
                        ...
                        BUSTUB_ENSURE(bustub->ExecuteSqlTxn(generate_select_sql(i), data_writer, txn),
                        ...
                        BUSTUB_ENSURE(bustub->ExecuteSqlTxn(generate_delete_sql(i), data_writer, txn),
                                      "cannot delete data");
