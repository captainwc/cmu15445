...
TEST(TxnExecutorTest, DISABLED_UpdateTestWithUndoLog) {  // NOLINT
    ...
    TxnMgrDbg("after insert, update, and commit", bustub->txn_manager_.get(), table_info.get(),
...
TEST(TxnExecutorTest, DISABLED_UpdateConflict) {  // NOLINT
...

    {
        ...
        auto txn5 = BeginTxn(*bustub, "txn5");
        WithTxn(txn5,
                QueryShowResult(*bustub, _var, _txn, "SELECT * FROM table1", IntResult{{0, 233, 0}, {1, 2333, 1}}));
