...
auto Int(uint32_t data) -> Value {
    return ValueFactory::GetIntegerValue(data);
}
...
void VerifyTuple(const Schema *schema, const Tuple &tuple, const std::vector<Value> &values) {
    for (uint32_t i = 0; i < schema->GetColumnCount(); i++) {
        ...
        if (!CompareValue(actual_val, expected_val)) {
            std::cerr << "tuple data mismatch: got " << tuple.ToString(schema) << "; at column " << i
                      << ", actual value " << actual_val.ToString() << " != expected value " << expected_val.ToString()
                      << std::endl;
            std::terminate();
        }
    }
...
void CheckUndoLogNum(BusTubInstance &instance, const std::string &txn_var_name, Transaction *txn, size_t expected_num) {
    ...
    if (undo_log_num != expected_num) {
        ...
        std::cerr
            << "hint: please refer to the writeup delete and update section. one transaction holds at most one undo "
...
void CheckUndoLogColumn(BusTubInstance &instance, const std::string &txn_var_name, Transaction *txn,
    ...
    if (undo_log_num != 1) {
        ...
        std::cerr
            << "hint: please refer to the writeup delete and update section. you should generate a partial tuple, "
               "keep adding things to the tuple when updating, and never remove things from it."
            << std::endl;
        std::terminate();
    }
    auto   undo_log = txn->GetUndoLog(0);
    size_t cnt      = std::count_if(undo_log.modified_fields_.begin(), undo_log.modified_fields_.end(),
    ...
    if (enable_length_check) {
        if (undo_log.tuple_.GetLength() > expected_columns * 4 /* integer width */) {
            fmt::println(stderr,
                         "Error: tuple length not correct? expecting <column> * 4, found {} while having {} columns",
...
void TableHeapEntryNoMoreThan(BusTubInstance &instance, TableInfo *table_info, size_t upper_limit) {
    ...
    if (cnt > upper_limit) {
        ...
        std::cerr << "hint: this is likely because you did not maintain the version chain correctly. you might have "
                     "implemented your update as delete + insert, so that a new tuple is created every time a tuple "
                     "gets updated."
                  << std::endl;
        std::terminate();
    }
...
void QueryIndex(BusTubInstance &instance, const std::string &txn_var_name, Transaction *txn, const std::string &query,
    ...
    for (size_t i = 0; i < expected_pk.size(); i++) {
        ...
        auto               res =
            instance.ExecuteSqlTxn(fmt::format("{} WHERE {} = {}", query, pk_column, expected_pk[i]), writer, txn);
        if (!res) {
            fmt::println(stderr, "{} failed to execute query when {} = {}", StatusFail("ERROR:"), pk_column,
                         expected_pk[i]);
            std::terminate();
        }
